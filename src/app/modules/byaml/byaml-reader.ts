import { NodeType, _NUL_CHAR, Endians} from "./types";
...
export class BymlParser {
	...
	constructor(buffer: ArrayBuffer) {
		...
		switch (magic) {
			...
			default: throw new Error('Invalid Header'); // PlaceHolder Error;
		}
	...
	parse(): any {
		...
		if (!this._isContainerType(nodeType)) {
			throw new Error('Not a container type') // PlaceHolder error;
		}
	...
	private _parseNode(nodeType: NodeType, offset: number) {
		...
		switch (nodeType) {
			...
			default: throw new Error('Either not a NodeType or unsupported node type: ' + nodeType + ' ' + offset)
		}
	}
	...
	private _parseStringTable(tableOffset: number): string[] {
		...
		if (nodeType !== NodeType.STRING_TABLE) {
			throw new Error('Invalid node type') // PlaceHolder Error;
		}
